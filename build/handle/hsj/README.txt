HANDLE.NET® Version 8.1 Software

HANDLE.NET 8.1 software is subject to the terms of the Handle.Net Public
License Agreement (version 1).  Please read the license:
http://hdl.handle.net/20.1000/103.

If you are upgrading from an earlier version of the software, see
UPGRADING FROM AN EARLIER VERSION below.

Note that CNRI no longer supports v6 and older software.


INSTALLATION

How to Install the Handle.Net (version 8.1) software:
...
1) Install Java version 6 or greater on your computer.  Note: if Java is
...
4) To set up and configure a new or existing server, run the following
   command.
      On Unix-like systems:
        /hs/hsj-8/bin/hdl-setup-server /hs/svr_1

      On Windows systems:
        \hs\hsj-8\bin\hdl-setup-server.bat \hs\svr_1
...
5) Once the configuration file has been generated, you will need to
   ...
   go to <http://hdl.handle.net/20.1000/111> to register to become
   a local handle service provider and then upload your file.  Also
   please read and agree to the Handle.Net Registry Service Agreement
   <http://hdl.handle.net/20.1000/102> and pay the associated fees
   for your prefix.  Note: if you have already registered and been
   allotted a prefix, contact the HNR Administrator for instructions
   on maintaining your prefix.
   ...
   will not be able to continue the install until you receive further
   information concerning your prefix.

   ONCE YOU RECEIVE YOUR PREFIX INFORMATION FROM THE HNR ADMINISTRATOR
   THEN PROCEED WITH THE FOLLOWING STEPS TO 'HOME' YOUR PREFIX TO YOUR
   NEW SERVICE.
...
6) Go to your svr_1 directory (where you installed the software) and
   ...
   with your prefix (as indicated in your email message from
   hdladmin@cnri.reston.va.us).  This allows anyone with the private
   key that matches your public key to be an administrator for your
   identifier and/or resolution services.

7) Start your handle server using the configuration generated in step 4.
   This can be done with the following command:

      On Unix-like systems:
        /hs/hsj-8/bin/hdl-server /hs/svr_1

      On Windows systems:
        \hs\hsj-8\bin\hdl-server.bat \hs\svr_1
...
Note: 'homing' the prefix can now done by configuration rather than by
sending a request using the Handle Tool client application.
...
9) The older GUI client Handle Tool can also be used.
   Start the Handle Tool using the following command.

      On Unix-like systems:
        /hs/hsj-8/bin/hdl-admintool

      On Windows systems:
        \hs\hsj-8\bin\hdl-admintool.bat
   ...
   For complete documentation on Handle.Net version 8.1 please refer
   to the Technical Manual (<hdl.handle.net/20.1000/105>).
...
Starting with version 7 and now with version 8.1, we have provided a
...
This can be done while the server is already running version 8.1.
...
non-human-readable binary format for this file; version 8.1 prefers a
human-readable and human-editable format. To make this change, use
this command
...
not strictly necessary, as version 8.1 clients can negotiate the
version; knowing the best version available in advance is an
optimization.)
